...
def login() -> tuple[Any, int] | Any:
    ...
    session_expiration = userC.find_user_by(session_id=session_id).session_expiration
    current_user = userC.find_user_by(email=email)
    # Prepare response JSON with session_id, email, and expiration time
    response_data = {
        "email": email,
        ...
        "expiration_time": session_expiration.isoformat(),  # Send as ISO format for better JS handling
        "message": "logged in"
    }

    # Prepare response with session cookie
    resp = jsonify(response_data)
    resp.set_cookie("session_id", session_id, expires=session_expiration)
